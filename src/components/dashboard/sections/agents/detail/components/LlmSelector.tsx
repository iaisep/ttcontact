...
const LlmSelector: React.FC<LlmSelectorProps> = ({
  ...
  return <Button variant="outline" className="flex items-center justify-between w-full max-w-full gap-1 sm:gap-2 bg-white text-gray-900 border-gray-200 rounded-full hover:bg-gray-50 px-2 sm:px-4 py-1 sm:py-2 h-auto overflow-hidden">
      <div className="flex items-center gap-1 sm:gap-2 overflow-hidden flex-wrap">
        ...
        <span className="truncate max-w-[100px] sm:max-w-[120px] text-[[10px]]">{currentLlm.name}</span>
      </div>
      ...
      <div className="flex items-center flex-shrink-0">
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end" className="w-[200px] z-50 bg-white">
            {LLM_OPTIONS.map(option => <DropdownMenuItem key={option.id} onClick={() => handleLlmChange(option.id)} className="flex items-center gap-2 text-xs sm:text-sm">

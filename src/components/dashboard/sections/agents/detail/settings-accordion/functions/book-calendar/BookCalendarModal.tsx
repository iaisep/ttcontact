...
interface BookCalendarModalProps {
  isOpen: boolean;
  onClose: () => void;
  agent: RetellAgent;
  onSuccess?: () => void;
...
const BookCalendarModal: React.FC<BookCalendarModalProps> = ({ 
  isOpen, 
  onClose, 
  agent,
  onSuccess,
  ...
  const [functionName, setFunctionName] = useState('book_appointment');
  const [description, setDescription] = useState('When users ask to book an appointment, book it on the calendar.');
  ...
  useEffect(() => {
    if (initialData && initialData.type === 'book_appointment_cal') {
      setFunctionName(initialData.name || 'book_appointment');
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Prepare the new calendar booking function
      const newFunction = {
        name: functionName,
      ...
      toast.success(initialData ? 'Calendar booking function updated successfully' : 'Calendar booking function added successfully');
      ...
      console.error('Error adding calendar booking function:', error);
      ...
      if (error.message && error.message.includes('Duplicate name found')) {
        ...
        toast.error('Failed to add calendar booking function');
      }
    } finally {
      setIsSubmitting(false);
    }
      ...
      <DialogContent className="sm:max-w-[500px]">
        <DialogHeader>
          <DialogTitle className="flex items-center">
            {initialData ? 'Edit Booking Function' : 'Book on the Calendar (Cal.com)'}
          </DialogTitle>
        </DialogHeader>
